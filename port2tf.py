...
def postprocess(pred_bbox, test_input_size, org_img_shape):
    ...
    # ***********************
    pred_prob = np.ones((pred_prob.shape[0], 1))
    # ***********************
...
def freeze_graph(checkpoint_path, output_node_names, savename):
    ...
    # output = YOLOV3(training).build_nework_slim_MNN(input_data,inputsize=INPUTSIZE)
...
def freeze_graph_test(pb_path, outnode):
    with tf.Graph().as_default():
        ...
        with tf.Session(config=gpuconfig) as sess:
            ...
            originimg = img
            orishape = img.shape
            img = tools.img_preprocess2(img, None, (INPUTSIZE, INPUTSIZE), False, keepratio=False)
            ...
            outbox = np.array(postprocess_nopad(outbox, INPUTSIZE, orishape[:2]))
...
if __name__ == '__main__':
    ...
    ckptpath = 'checkpoints/weights/yolo.ckpt-60'
